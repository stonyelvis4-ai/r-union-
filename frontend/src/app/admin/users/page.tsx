    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50 dark:bg-slate-950 px-4 py-10 text-gray-900 dark:text-slate-50">
        <div className="w-full max-w-md rounded-2xl border border-rose-500/40 bg-white dark:bg-slate-900/80 p-6 text-center shadow-lg">
          ...
          <Link
            href="/"
            className="mt-4 inline-flex items-center gap-2 text-xs font-medium text-sky-500 dark:text-sky-300 hover:text-sky-400 dark:hover:text-sky-200"
          >
      ...
      <div className="mx-auto max-w-5xl">
        ...
        {loading ? (
          ...
          <p className="mt-8 text-sm text-gray-500 dark:text-slate-300">
            Aucun participant dans votre espace.
          </p>
        ) : (
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-slate-700/80 text-xs uppercase tracking-wide text-gray-400 dark:text-slate-400">
                  ...
                  <th className="px-4 py-3">
                    <span className="inline-flex items-center gap-1">👤 Nom</span>
                  </th>
                  <th className="px-4 py-3">
                    <span className="inline-flex items-center gap-1">🏷️ Rôle</span>
                  </th>
                  <th className="px-4 py-3">
                    <span className="inline-flex items-center gap-1">🔘 Actif</span>
                  </th>
                  <th className="px-4 py-3">
              ...
              <tbody>
                {users.map((u) => (
                  <tr
                    key={u.id}
                    className="border-t border-gray-200 dark:border-slate-800/60 transition hover:bg-gray-50 dark:hover:bg-slate-800/40"
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center gap-1 text-xs ${u.isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}
                      >
                        {u.isActive ? '✅ Oui' : '❌ Non'}
                      </span>
                    </td>
